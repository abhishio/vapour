...
def sendto(msg):
	emsg=encrypt(msg)
	s.sendto(emsg,(server,50301))
def recvf():
	r=dcrypt(s.recv(1000))
	return r

LoginStatus=cmd.getstatusoutput('cat vapourbox/blogin.txt')[1]
top=tk.Tk()
...
def CONNECT():
	os.system('python3.4 vapourbox/open.py')
def DISCONNECT():
	os.system('python3.4 vapourbox/disconnect.py')
def expsize():
	subprocess.Popen(["ssh","-X","root@192.168.122.234","lvextend", "--size" ,"+%s"%(expandSB.get()), "/dev/box/box1"])
	subprocess.Popen(["ssh","-X","root@192.168.122.234","/root/extend.sh"])
def loginfn():
	...
	if info['uname']==''or info['passwd']=='':
		...
		LoginStatus=recvf()
		if LoginStatus=='True':
			os.system('echo "True" > vapourbox/blogin.txt')
			userName=info['uname']
...
def logout():
	global LoginStatus
	LoginStatus='False'
	os.system('echo "False" > vapourbox/blogin.txt')		
	root.tkraise()
...
label=tk.Label(root, text='Vapour Box', font='Helvetica -24 bold').pack(fill=tk.Y, expand=1,side=tk.TOP,padx=2,pady=2)
...
Connect=tk.Button(loggedFrame1, text='Connect', command=CONNECT, bg='goldenrod1', fg='blue', activeforeground='white', activebackground='grey').pack(fill=tk.X,expand=1,side=tk.LEFT,padx=2,pady=2)
Disconnect=tk.Button(loggedFrame1, text='Disconnect', command=DISCONNECT, bg='goldenrod1', fg='blue', activeforeground='white', activebackground='grey').pack(fill=tk.X,expand=1,side=tk.LEFT,padx=2,pady=2)
manageBox=tk.Button(loggedFrame1, text='Manage', command=manage.tkraise, bg='goldenrod1', fg='blue', activeforeground='white', activebackground='grey').pack(fill=tk.X,expand=1,side=tk.RIGHT,padx=2,pady=2)
quit=tk.Button(loggedFrame2, text='Logout', command=logout, bg='red', fg='blue', activeforeground='white', activebackground='green').pack(padx=2,pady=2)
loggedFrame1.pack()
loggedFrame2.pack()
...
expandBtn2=tk.Button(expandFrame2, text="Expand",command=expsize).pack(side=tk.LEFT,padx=2,pady=2)
